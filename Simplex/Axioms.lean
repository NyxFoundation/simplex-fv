...
/-- **Leader-rotation tail bound (Theorem 3.3).** In a random
    ...
    negligible once `k = ω(log λ)`. Declared as an axiom. Source: Chan & Pass,
...
/-- **Expected leader-offset bound (Theorem 3.4).** The offset
    ...
    `[0, 1/2]`. Declared as axioms. Source:
